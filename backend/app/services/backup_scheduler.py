...
# ============= OAuth-Based File Backups =============

@celery_app.task(name="backup.files.daily")
def backup_agency_files_daily(agency_id: str = None):
    """
    OAuth-based backup: Daily backup of uploaded files to Google Drive
    Runs for agencies with OAuth Google Drive connection
    """
    from app.services.google_drive import backup_agency_files_to_gdrive
    
    db = SessionLocal()
    try:
        agencies = db.query(Agency).filter(
            Agency.gdrive_refresh_token.isnot(None),
            Agency.gdrive_folder_id.isnot(None),
            Agency.status == "active",
        ).all()
        ...
        for agency in agencies:
            try:
                success, message = backup_agency_files_to_gdrive(db, str(agency.id))
                if success:
                    results['success'] += 1
                    logger.info(f"✅ Daily file backup successful for {agency.name}: {message}")
                else:
                    results['failed'] += 1
                    results['errors'].append(f"{agency.name}: {message}")
                    logger.warning(f"⚠️ Daily file backup failed for {agency.name}: {message}")
            except Exception as e:
                results['failed'] += 1
                results['errors'].append(f"{agency.name}: {str(e)}")
                logger.error(f"❌ Daily file backup error for {agency.name}: {str(e)}")
        
        logger.info(f"Daily file backup task completed: {results['success']}/{results['total']} successful")
        return results
        
    except Exception as e:
        logger.error(f"❌ Daily file backup task failed: {str(e)}")
        return {'error': str(e)}
    finally:
        db.close()
...
def cleanup_old_backups(days_to_keep: int = 90):
    """
    Clean up old backup records (older than specified days)
    Prevents unlimited database storage usage
    """
    ...
    try:
        ...
        old_backups = db.query(Backup).filter(
            ...
            Backup.gdrive_file_id.isnot(None)
        ).all()
            ...
            try:
                ...
                if agency and agency.gdrive_refresh_token:
                    try:
                        ...
                        logger.warning(f"Failed to delete remote file: {str(e)}")
