...
export default function Workers() {
    ...
    const deleteWorker = async (id, name) => {
        if (!confirm(`Delete worker "${name}"? Their route assignments will also be removed.`)) return;
        try {
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        <div className="px-6 py-3 border-b border-slate-200">
                            <div className="relative">
                                ...
                                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search workers..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
                            </div>
                        </div>
                        {isLoading ? (
                                            ...
                                            <td className="px-6 py-4 flex items-center gap-3">
                                                ...
                                                {editId === worker.id ? (
                                                    <div className="space-y-1">
                                                        ...
                                                        <input type="password" placeholder="New password (optional)" value={editData.password} onChange={e => setEditData({ ...editData, password: e.target.value })} className="border border-indigo-300 rounded px-2 py-1 text-sm w-40" />
                                                    </div>
                                                ) : (
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                {editId === worker.id ? (
                                                    <div className="flex items-center justify-end gap-2">
                                                        <button onClick={saveEdit} className="text-xs bg-indigo-600 text-white px-3 py-1 rounded-lg hover:bg-indigo-700">Save</button>
